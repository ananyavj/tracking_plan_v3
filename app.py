...
if st.session_state.audit_summary:
    ...
    if st.session_state.ai_analysis and "html_report" in st.session_state.ai_analysis:
        ...
        report = st.session_state.ai_analysis
        ...
        # ── Render report inline (no iframe, no black void) ──────────────────
        summary_text = report.get("summary") or report.get("executive_summary", "")
        recs  = report.get("recommendations", [])
        gaps  = report.get("tracking_plan_gaps", [])

        st.markdown("""
<style>
.kai-header{display:flex;justify-content:space-between;align-items:flex-start;
  border-bottom:1px solid #1e2535;padding-bottom:1rem;margin-bottom:1.5rem}
.kai-title{font-size:1.3rem;font-weight:700;color:#f1f5f9}
.kai-title span{background:linear-gradient(90deg,#4facfe,#00f2fe);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent}
.kai-meta{font-size:11px;color:#475569;text-align:right;line-height:1.8}
.kai-meta b{color:#64748b}
.kai-label{font-size:0.72rem;font-weight:600;text-transform:uppercase;
  letter-spacing:1px;color:#64748b;margin:1.5rem 0 0.75rem}
.kai-summary-box{background:#131720;border:1px solid #1e2d3d;
  border-left:3px solid #4facfe;padding:1rem 1.25rem;
  border-radius:8px;color:#94a3b8;font-size:13.5px;line-height:1.7;margin-bottom:1rem}
.kai-rec-card{display:flex;gap:1rem;background:#131720;border:1px solid #1e2535;
  border-radius:10px;padding:1rem 1.25rem;margin-bottom:0.75rem}
.kai-rec-num{flex-shrink:0;width:26px;height:26px;border-radius:50%;
...
.kai-rec-title{font-weight:600;color:#e2e8f0;font-size:13.5px;margin-bottom:0.25rem}
.kai-rec-detail{color:#94a3b8;font-size:13px;line-height:1.6}
.kai-code{background:#0a0d12;border:1px solid #1e2535;border-radius:6px;
  padding:0.75rem 1rem;margin-top:0.5rem;font-family:'JetBrains Mono',monospace;
  font-size:12px;color:#7dd3fc;line-height:1.6;overflow-x:auto;white-space:pre}
.kai-gap-table{width:100%;border-collapse:collapse;margin-top:0.5rem}
.kai-gap-table th{font-size:11px;font-weight:600;text-transform:uppercase;
  letter-spacing:.8px;color:#475569;padding:0.5rem 0.75rem;
  border-bottom:1px solid #1e2535;text-align:left}
.kai-gap-table td{padding:0.65rem 0.75rem;border-bottom:1px solid #141c28;
  font-size:13px;color:#94a3b8;vertical-align:top}
.kai-gap-table tr:last-child td{border-bottom:none}
.kai-gap-event{font-family:monospace;color:#a5b4fc;font-size:12.5px}
.kai-badge{display:inline-block;padding:.15rem .55rem;border-radius:20px;
  font-size:10.5px;font-weight:600;letter-spacing:.5px;white-space:nowrap}
</style>""", unsafe_allow_html=True)

        # Header
        verified_flag = meta.get("verified", False)
        ext_history   = meta.get("external_history_used", False)
        verified_tag  = " ✅ Ground-Verified" if (verified_flag and not ext_history) else (" 🔍 +Supplemental" if verified_flag else "")
        st.markdown(f"""
<div class="kai-header">
  <div class="kai-title">🤖 <span>Kaliper</span> Diagnostic Report</div>
  <div class="kai-meta">
    <b>Model</b> {meta.get('model','—')}<br>
    <b>Iterations</b> {meta.get('iterations','—')} · <b>Tools</b> {meta.get('tool_calls','—')}{verified_tag}
  </div>
</div>""", unsafe_allow_html=True)

        # Summary
        if summary_text:
            st.markdown('<div class="kai-label">📊 Executive Summary</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="kai-summary-box">{summary_text}</div>', unsafe_allow_html=True)

        # Recommendations
        if recs:
            st.markdown('<div class="kai-label">🔧 Recommendations</div>', unsafe_allow_html=True)
            for i, r in enumerate(recs):
                if isinstance(r, str):
                    title, detail, code_fix = "Recommendation", r, ""
                else:
                ...
                code_block = f'<div class="kai-code">{code_fix}</div>' if code_fix else ""
                st.markdown(f"""
<div class="kai-rec-card">
  <div class="kai-rec-num">{i+1}</div>
  <div style="flex:1;min-width:0">
    <div class="kai-rec-title">{title}</div>
    <div class="kai-rec-detail">{detail}</div>
    {code_block}
  </div>
</div>""", unsafe_allow_html=True)
        ...
        # Tracking Plan Gaps
        VERDICT_STYLES = {
            "typo":                 ("#f59e0b", "rgba(245,158,11,0.12)",  "TYPO"),
            "new_feature":          ("#38bdf8", "rgba(56,189,248,0.12)",  "NEW FEATURE"),
            "test_artifact":        ("#94a3b8", "rgba(148,163,184,0.10)", "TEST ARTIFACT"),
            "not_in_tracking_plan": ("#f87171", "rgba(248,113,113,0.12)", "NOT IN PLAN"),
        }
        if gaps:
            st.markdown('<div class="kai-label">📋 Tracking Plan Gaps</div>', unsafe_allow_html=True)
            rows = ""
            for g in gaps:
                name    = g.get("event_name", "Unknown")
                verdict = g.get("verdict", "unknown")
                reason  = g.get("reason", "")
                col, bg, label = VERDICT_STYLES.get(verdict, ("#e94560", "rgba(233,69,96,0.10)", verdict.upper()))
                badge = f"<span class='kai-badge' style='color:{col};background:{bg}'>{label}</span>"
                rows += f"<tr><td class='kai-gap-event'>{name}</td><td>{badge}</td><td>{reason}</td></tr>"
            st.markdown(f"""
<table class="kai-gap-table">
  ...
  <tbody>{rows}</tbody>
    ...
    # NOTE: Tracking plan gaps are already rendered inside the HTML report above.
    # The block below is intentionally removed to avoid duplicate rendering.
